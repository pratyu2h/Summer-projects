...
def train(data_root, epochs=10, batch_size=32, lr=1e-4, image_size=224,
    ...
    if not (data_root / "Data_Entry_2017.csv").exists():
        raise FileNotFoundError(
            f"Dataset CSV not found at {data_root / 'Data_Entry_2017.csv'}. "
            "Make sure --data-root points to the NIH ChestX-ray data directory."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-root", default=".",
                        help="Path to the NIH data root directory (default: current working directory)")
